...
from deluge_compat.functions import (
    getUrl, postUrl, encodeUrl, urlDecode, urlEncode,
    base64Encode, base64Decode, aesEncode, aesDecode,
    abs_func, cos_func, sin_func, tan_func, log_func,
    min_func, max_func, exp_func, power_func, round_func,
    sqrt_func, toDecimal, toHex, ceil_func, floor_func,
    randomNumber, info, ifnull, replaceAll,
    BUILTIN_FUNCTIONS
)
from deluge_compat.types import Map, List, deluge_string
...
class TestBuiltinFunctionsList:
    ...
    def test_all_functions_in_builtin_dict(self):
        ...
        expected_functions = [
            'getUrl', 'postUrl', 'encodeUrl', 'urlEncode', 'urlDecode',
            'base64Encode', 'base64Decode', 'aesEncode', 'aesDecode',
            'abs', 'cos', 'sin', 'tan', 'log', 'min', 'max', 'exp',
            'power', 'round', 'sqrt', 'toDecimal', 'toHex', 'ceil',
            'floor', 'randomNumber', 'info', 'Map', 'List', 'ifnull',
